...
class XhamRequete {
  ...
  function truncateTable () {
    ...
    if ( $sql['error'] AND isset ( $xham ) ) $xham -> addErreur ( "clRequete : ".$sql['error']."<br/>Requête : ".$sql['requete'] ) ;
    $fin = $this->temps();
    $total = $fin - $debut;
    $total = substr($total,0,8);
    if ( DEBUGSQL ) eko ( "Temps d'exécution de la requête : $total ($truncate)" ) ;
    $xham->nbRequetes++ ;
  ...
  function delRecord ( $cw='jambon' ) {
    ...
    $total = $fin - $debut;
    $total = substr($total,0,8);
    if ( DEBUGSQL ) eko ( "Temps d'exécution de la requête : $total ($delete)" ) ;
    $xham->nbRequetes++ ;
    $xham->tpRequetes += $total ;
    return $sql ;
  }
  ...
  function updRecord ( $cw='jambon' ) {
	...
	function exec_requete($req,$format = 'sql') {
		...
		$sql['errno'] = mysql_errno() ;
		$sql['error'] = mysql_error() ;
		if ( $sql['error'] AND is_object ( $xham ) AND ! defined('NOERROR')) $xham -> addErreur ( "clRequete : ".$sql['error']."<br/>Requête : ".$req ) ;
		//print affTab ( $sql ) ;
		$fin = $this->temps();
		$total = $fin - $debut;
		$total = substr($total,0,8);
...
/*tente l'update et si ça marche pas, fait une insert*/
  ...
  function formatData ( ) {
    ...
    while ( list ( $key, $val ) = each ( $data ) ) {
      ...
      if (  $desc[$key][1] != 'blob' AND $desc[$key][0] < strlen ( $val ) AND isset ( $xham )  AND ! $this->isCutData AND $desc[$key][1] != "blob" ) { // Donnée trop longue
	$erreur = "$key : <br/>Longueur attendue : ".$desc[$key][0]." <br/>Longueur reçue : ".strlen($val)."<br/>Contenu : ".str_replace(' ','_',$val).'.' ;
	$xham->addErreur ( "Base -> ".$this->db."<br/>Table -> ".$this->table."<br/>Erreur -> Problème de longueur d'une chaîne<br/>$erreur<br>" ) ;
      }
  ...
  function descTable ( ) {
    ...
    if ( ( ! isset ( $this->db ) or ! isset ( $this->table ) ) AND isset ( $xham ) ) {
      ...
      if ( ! $infotable AND isset ( $xham ) ) $xham->addErreur ( "Base -> ".$this->db."<br/>Table -> ".$this->table."<br/>Erreur -> Problème d'accès aux informations de la table.<br>" ) ;
	...
	if ( $sql['error'] AND isset ( $xham ) )
		$xham -> addErreur ( "clRequete : ".$sql['error']."<br/>Requête : ".$requete ) ;	
    }
