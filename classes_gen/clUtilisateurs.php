...
class clUtilisateurs {
  ...
  function __construct ( ) {
    global $session ;
    if ( $session -> getNavi ( 2 ) == "ChangementPassword" ) {
      $this->af .= $this -> changerPassword ( ) ;
    } else {
      if ( ( $_POST['Enlever'] OR $_POST['Enlever_x'] ) AND $session -> getDroit ( "Configuration_Utilisateurs", "m" ) ) {
	$this -> enleverGroupes ( ) ;
      } elseif ( ( $_POST['Ajouter'] OR $_POST['Ajouter_x'] ) AND $session -> getDroit ( "Configuration_Utilisateurs", "m" ) ) {
	$this -> ajouterGroupes ( ) ;
      } elseif ( ( $_POST['Modifier'] OR $_POST['Modifier_x'] ) AND $session -> getDroit ( "Configuration_Utilisateurs", "m" ) ) {
	$this -> modifierUtilisateur ( ) ;
      } elseif ( ( $_POST['Supprimer'] OR $_POST['Supprimer_x'] ) AND $session -> getDroit ( "Configuration_Utilisateurs", "d" ) ) {
	$this -> supprimerUtilisateur ( ) ;
      }  elseif ( ( $_POST['AjouterUtilisateur'] OR $_POST['AjouterUtilisateur_x'] ) AND $session -> getDroit ( "Configuration_Utilisateurs", "w" ) ) {
  ...
  function changerPassword ( ) {
    ...
    if ( $_POST['Modifier'] OR $_POST['Modifier_x'] ) {
      $param[cw] = "WHERE uid='".$session -> getUid ( )."' AND password=MD5('".$_POST['pwd']."')" ;
	...
	if ( $_POST['pwd1'] AND ( $_POST['pwd1'] == $_POST['pwd2'] ) ) {
	  ...
	  $param[set] = $p ;
	  $req = new clResultQuery ;
	  $res = $req -> Execute ( "Fichier", "updateUtilisateur", $param, "ResultQuery" ) ;
  ...
  function ajouterUtilisateur ( ) {
    ...
    if ( ! $this->erreurs ) {
      ...
      $req = new clResultQuery ;
      $res = $req -> Execute ( "Fichier", "addUtilisateur", $param, "ResultQuery" ) ;
  ...
  function enleverGroupes ( ) {
	...
	if ( $groupes ) $groupes .= ",".$val ;
      ...
      $requete = new clRequete ( BASEXHAM, TABLERELUG ) ;
  ...
  function ajouterGroupes ( ) {
	...
	if ( ! $res[INDIC_SVC][2] ) {
	  ...
	  $requete = new clRequete ( BASEXHAM, TABLERELUG, $data ) ;
	  $requete->addRecord ( ) ;
	}
      }
    }
  ...
  function supprimerUtilisateur ( ) {
    ...
    if ( $session -> getNavi ( 2 ) == "ValiderSupprimer" ) {
      ...
      // Variable de navigation.
      $mod -> MxHidden ( "hidden", "navi=".$session->genNavi ( $session->getNavi ( 0 ), $session->getNavi ( 1 ), "ValiderSupprimer" )."&iduser=".$_POST['iduser'] ) ;
      // On retourne le code HTML généré.
  ...
  function genAffichage ( ) {
    global $session ;
    if ( $session -> getDroit ( "Configuration_Utilisateurs", "r" ) ) {
	...
	if ( $_POST['iduser'] ) {
	  ...
	  if ( $res[INDIC_SVC][2] ) {
	    ...
	    if ( ! $session -> getDroit ( "Configuration_Utilisateurs", "m" ) ) {
	    ...
	    if ( ! $session -> getDroit ( "Configuration_Utilisateurs", "d" ) ) {
