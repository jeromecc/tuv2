...
class XhamGestionGroupes {
  ...
  function affecterGroupes ( ) {
	...
	$req = new XhamRequete ( BASEXHAM, TABLERELAG, $data ) ;
	$ris = $req -> addRecord ( ) ;
      }
    }
  }
  ...
  function enleverGroupes ( ) {
    ...
    for ( $i = 0 ; isset ( $liste[$i] ) ; $i++ ) {
      $req = new XhamRequete ( BASEXHAM, TABLERELAG, (isset($data)?$data:array()) ) ;
  ...
  function genAffichage ( ) {
    if ( isset ( $_POST['Annuler'] ) OR isset ( $_POST['Annuler_x'] ) ) {
      $_POST['idGroupe'] = '' ;
    } elseif ( $this->xham -> getDroit ( "Configuration_Groupes", "w" ) AND ( isset ( $_POST['Ajouter'] ) OR isset ( $_POST['Ajouter_x'] ) ) ) {
      $text = $this->addGroupe ( ) ;
    } elseif ( $this->xham -> getDroit ( "Configuration_Groupes", "m" ) AND ( isset ( $_POST['Modifier'] ) OR isset ( $_POST['Modifier_x'] ) ) ) {
      $text = $this->modGroupeV ( ) ;
      $_POST['idGroupe'] = '' ;
    } elseif ( $this->xham -> getDroit ( "Configuration_Groupes", "d" ) AND ( isset ( $_POST['Supprimer'] ) OR isset ( $_POST['Supprimer_x'] ) ) ) {
      $text = $this->delGroupe ( ) ;
      $_POST['idGroupe'] = '' ;
    } elseif ( $this->xham -> getDroit ( "Configuration_Groupes", "m" ) AND isset ( $_POST['idGroupe'] ) ) {
      $text = $this->modGroupe ( ) ;
    }
    if ( ( isset ( $_POST['Affecter'] ) OR isset ( $_POST['Affecter_x'] ) ) AND $this->xham -> getDroit ( "Configuration_Groupes", "w" ) ) $this->affecterGroupes ( ) ;
    if ( ( isset ( $_POST['Enlever'] ) OR isset ( $_POST['Enlever_x'] ) ) AND $this->xham -> getDroit ( "Configuration_Groupes", "d" ) ) $this->enleverGroupes ( ) ;
    ...
    if ( ! $this->xham -> getDroit ( "Configuration_Groupes", "w" ) ) $mod -> MxBloc ( "nouveau", "modify", " " ) ;
  ...
  function getGroupesNonAffectes ( $listeGroupes=array ( ) ) {
    ...
    if ( isset($tab) &&  is_array ( $tab ) ) return $tab ;
    else return array ( ) ;
  }
  ...
  function modGroupe ( ) {
    ...
    if ( ! $this->xham -> getDroit ( "Configuration_Groupes", "m" ) ) $mod -> MxBloc ( "modifier", "modify", " " ) ;
    if ( ! $this->xham -> getDroit ( "Configuration_Groupes", "d" ) ) $mod -> MxBloc ( "supprimer", "modify", " " ) ;
    return $mod -> MxWrite ( "1" ) ;
  }
