...
class XhamQuery {
  ...
  function get_file_content ( $filename ) {
    ...
    if ( file_exists ( "queries_int/".$filename ) ) {
      ...
      $rep = "queries_gen/" ;
    } elseif ( file_exists ( URLLOCAL."queries_int/".$filename ) ) {  
      $fd = fopen ( $relocate."queries_int/".$filename, "r" ) ;
      $rep = URLLOCAL."queries_int/" ;
    } elseif ( file_exists ( URLLOCAL."queries_gen/".$filename ) ) {
       $fd = fopen ( $relocate."queries_gen/".$filename, "r" ) ;
      $rep = URLLOCAL."queries_gen/" ;
    } else  {
      //echo "pas trouvé";
      fopen ( "index.php", "r" ) ;
    }
  ...
  function Execute ( $type_entree, $requete, $param, $type_sortie='ResultQuery' ) {
	...
	for ( $i = 0 ; $i < $res['INDIC_SVC'][2] ; $i++ ) {
      ...
      $lib_indic_svc = array ( 'num_err'=>$res['INDIC_SVC'][0], 'lib_err'=>$res['INDIC_SVC'][1], 'affected_rows'=>$res['INDIC_SVC'][2], 'id_created'=>$res['INDIC_SVC'][3] ) ;
      ...
      $xml_data = "<result num='".$res['INDIC_SVC'][2]."'>".$xml_data.$xml_indic_svc."</result>" ;
  ...
  function UseFile ( $requete, $Param, $find="NOM" ) {
    ...
    if ( isset ( $xham ) AND $resultats['INDIC_SVC'][1] ) $xham -> addErreur (  "clResultQuery :<br>".$resultats['INDIC_SVC'][0]."<br>".$resultats['INDIC_SVC'][1]."<br>".$resultats['INDIC_SVC'][2]."<br>".$resultats['INDIC_SVC'][15] ) ;
  ...
  function get_xml ( $requete, $Param, $find="NOM" ) {
    ...
    if ( sizeof ( $tags ) > 0 ) {
      for ( $i = 0 ; $i < $Resultats['INDIC_SVC'][2] ; $i++ ) {
	$xml_data .= "<element>" ;
	while ( list ( $key, $val ) = each ( $tags ) ) {
	  $col = $$val ;
	  ( ! isset ( $col[$i] ) ) ? $col[$i] = "" : "" ;
  ...
  function execute_request ( $value_query, $value_config, $Param ) {
    ...
    case "MySQL":
      ...
      if ( ! $conn ) {
	global $xham ;
	if ( $xham ) $xham -> addErreur ( "La connexion au serveur MySQL \"".$value_config['host']."\" avec l'utilisateur \"".$value_config['login']."\" et le mot de passe \"*******\" a échoué." ) ;
      }
	...
	$INDIC_SVC[0] = mysql_errno ( ) ;
	$INDIC_SVC[1] = mysql_error ( ) ;
	if( is_object($xham) && $INDIC_SVC[1]) { eko($xham ->errs->whereAmI()) ; eko($requete); }
