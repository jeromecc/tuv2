 ...
 */
	...
	/*26 janv. 2006, 1.28 patch1 modified by Emmanuel Cervetti ecervetti@ch-hyeres.fr 
	 * objet modif: permettre conservation de la page courante dans les données
	 * $_SESSION si la page courante n'est pas passée en GET
	 * */
...
private $title ;
 ...
 function ListMaker ($tpl='', $self_url='', $use_cookies=false) {
	 global $HTTP_COOKIE_VARS, $_GET, $HTTP_SERVER_VARS;
 ...
 */
 ...
 function getList ($nb_result_per_page=0, $tpl='') {
	...
	if (empty ($_GET['sort']) ) {
		...
		$current_sort = urldecode($_GET['sort']);
	}
	...
	if ($item_count < 1) {
		...
		/*26 janv. 2006, modified by Emmanuel Cervetti ecervetti@ch-hyeres.fr */
		//modif jumelle dans methode getListDB
		if (isset($_GET['page']) && !empty($_GET['page']) ) {
			$this->PageNumber = $_GET['page'];
			$_SESSION['listmaker_current_page'] = $this->PageNumber ; 
			$_SESSION['listmaker_current_template'] = $this->templatefile;
			$_SESSION['listmaker_current_titre'] = $this->title;
		} elseif (isset($_SESSION['listmaker_current_page']) && $_SESSION['listmaker_current_page']) {
			if($_SESSION['listmaker_current_template'] == $this->templatefile && $_SESSION['listmaker_current_titre'] == $this->title) {
				$this->PageNumber = $_SESSION['listmaker_current_page'] ;			
			} else {
				$this->PageNumber = 1;
				$_SESSION['listmaker_current_page'] = 1 ;
				$_SESSION['listmaker_current_template'] = $this->templatefile;
				$_SESSION['listmaker_current_titre'] = $this->title;
			}
		} else {
 ...
 function dbGetList ($query, $nb_result_per_page=0, $tpl='') {
	...
	if ($nb_result_per_page>0) {
		...
		/*26 janv. 2006, modified by Emmanuel Cervetti ecervetti@ch-hyeres.fr */
		//modif jumelle dans methode getList
		if (isset($_GET['page']) && !empty($_GET['page']) ) {
			$this->PageNumber = $_GET['page'];
			$_SESSION['listmaker_current_page'] = $this->PageNumber ; 
			$_SESSION['listmaker_current_template'] = $this->templatefile;
