...
// Si les fichiers de configuration des bases n'existent pas, on les crée.
if ( ! file_exists ( "queries_gen/config_xham.cfg" ) ) {
    ...
    fwrite ( $FIC, $dom->saveXML ( ) ) ;
    fclose ( $FIC ) ;
}
if ( ! file_exists ( "queries_gen/config_ccam.cfg" ) ) {
...
if ( ! file_exists ( "queries_gen/config_terminal.cfg" ) ) {
	$dom = new DOMDocument ( '1.0', 'utf8' ) ;
	$result = $dom->createElement ( 'result', '' ) ;
	$result -> setAttribute ( 'num', '1' ) ;
	$element = $dom->createElement ( 'element', '' ) ;
...
if ( ! file_exists ( "queries_gen/config_formx.cfg" ) ) {
        $dom = new DOMDocument ( '1.0', 'utf8' ) ;
        $result = $dom->createElement ( 'result', '' ) ;
        $result -> setAttribute ( 'num', '1' ) ;
        $element = $dom->createElement ( 'element', '' ) ;
