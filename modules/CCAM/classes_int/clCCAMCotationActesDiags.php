...
$this->typeDest=$paramCCAM[typeDest];
...
$this->dest_attendue = $paramCCAM[dest_attendue];
}
	...
	if ($options->getOption("ActiverModuleActes")){
		if ($session->getDroit("CCAM_ACTES_".$this->typeListe,"r")){
			...
			if ($res[INDIC_SVC][2]==0){
				...
				if ( $this->typeDest != 'H' AND $this->numUFdem != $options->getOption('numUFUHCD') AND $this->numUFdem != $options->getOption('numUFSC' ) AND $this->numUFdem != $options->getOption('numUFUHCDrepere' ) )
					$mod_recap->MxBloc("afficheActesDiag.afficheActe","replace","<table width=\"100%\"><tr><th>Code Acte</th><th>M&eacute;decin</th></tr><tr><td colspan=2 style=\"color:red;text-align:center;\">Aucun acte saisi</td></tr></table>");
				else $mod_recap->MxBloc("afficheActesDiag.afficheActe","replace","<table width=\"100%\"><tr><th>Code Acte</th><th>M&eacute;decin</th></tr><tr><td colspan=2 style=\"color:green;text-align:center;\">Les actes sont facultatifs sur ce dossier</td></tr></table>");
			}
			else{
...
if (!$sortir){
	...
	unset($param);
	$param[idDomaine] = CCAM_IDDOMAINE;
	$optionTri = $options->getOption("Catégories Diagnostics");
  if ( $optionTri == "Manuel" ) $tri = "rang";
	elseif ( $optionTri == "Alphabétique" ) $tri = "identifiant";
	elseif ( $optionTri == "Alphabétique inversé" ) $tri = "identifiant desc";
	else $tri = "identifiant";
  $param[order] = $tri;
  ...
  //eko($tabListeSelection1);
	...
	if ($options->getOption("ActiverModuleActes") and $session->getDroit("CCAM_ACTES_ACTES","r")){
		...
		$tabListeSelection2[AMI] ="Actes Infirmiers";
...
//eko ($this->typeIntervenant);	
      ...
      if ($idListeSelection2=="tous"){
        ...
        $requete="CCAM_getAutresActesNonListe";
			...
			elseif ($idListeSelection2=="AMI"){
        $paramA[type]="NGAP";
        $paramA[cw]="";
        $requete="CCAM_getAMIActes";
      }
			else{
      ...
      $tabListeGauche=$alltableau;
		...
		$mod->MxSelect("actesBlocGauche.idListeSelection2","idListeSelection2",$idListeSelection2,
			$tabListeSelection2,'','',"onChange=reload(this.form) size=\"5\"");
    ...
    // VERSION 1
    /* Construction de la liste des anesthesiste */
    /* liste des medecins urgentistes */
    /*$listeGen = new clListesGenerales ( "recup" ) ;
    $listeMotifs = $listeGen ->getListeItemsv2 ( "Médecins", "1", '', "libre");
    eko($listeMotifs);
    $tabAnesth = $listeMotifs;
    eko($tabAnesth);
    /* liste des medecins anesthesistes */
    
    // VERSION 2
    
    // On utilise la table ccam_anesthesie_domaine
    $req = new clResultQuery;
    $res = $req->Execute("Fichier","CCAM_getMedecinADomaine2",$param,"ResultQuery");
    //eko($res);
    
    for ( $i=0 ; $i < $res["INDIC_SVC"][2] ; $i++ ) {
      $tabAnesth[$res["identifiant"][$i]] = $res["libelle"][$i];
      }
    ...
    (!isset($_POST['anesthesiste']))?$anesthesiste=$matriculeUrgentiste:$anesthesiste=$_POST['anesthesiste'];
		...
		if ( ! $session->getDroit ( 'CCAM_ACTES_Presents', 'm' ) ) {
...
function add1Acte2( ){
  ...
  global $options;
	eko("entrée dans add1acte2 ");
  $liste_CCAM    = explode (",",$options->getOption("Activite4_ListeActeCCAM_CCAMNonCode4"));
  for ( $i = 0 ; isset($liste_CCAM[$i]) ; $i++ ) {
    $paramRq[cw] = "idEvent=".$this->idEvent." and idDomaine=".CCAM_IDDOMAINE." and codeActe='".$liste_CCAM[$i]."'";
    $req         = new clResultQuery;
    $res         = $req->Execute("Fichier","CCAM_getActesDiagsCotation",$paramRq,"ResultQuery");

    if ( $res["INDIC_SVC"][2] == 0 ) {
      unset($paramRq);
      $paramRq[idActe] = $liste_CCAM[$i];
      $paramRq[cw]     = "and idDomaine=".CCAM_IDDOMAINE;
      $req3            = new clResultQuery;
      $res3            = $req3->Execute("Fichier","CCAM_get1Acte",$paramRq,"ResultQuery"); 
      
      unset($param);
      $param[codeActe]             = $liste_CCAM[$i];
  ...
  if (substr($codeActe,0,4)!="NGAP"){
    ...
    // elseif (($heure<9 or $heure>=20) and $modificateursOK[U]) $modificateurs="U";
        ...
        // Médecin urgentiste
        $res2=$req->Execute("Fichier","CCAM_getNomMedURG",$paramRq,"ResultQuery");
  			$nomMed4=$res2["nomitem"][0];
  			
        // Médecin spécialiste
  			if ( $res2[INDIC_SVC][2] == 0 ) {
          $res2=$req->Execute("Fichier","CCAM_getNomMedSPEC",$paramRq,"ResultQuery");
          $nomMed4=$res2["nomitem"][0];
        }
...
if ( $activ4!="O" and $_POST['anesthesiste']!="aucun#" and $res4["INDIC_SVC"][2] == 1 ) {
    
    if ( $options->getOption("Activite4_AjoutActeCCAM_CCAMNonCode4") ) {
        $this->add1Acte2( );
    }
  }
...
if ( ! $options->getOption("Activite4_desactiver_controle")  and $activ4=="O" and $_POST['anesthesiste']=="aucun#") {
  ...
  /* Suivant l'option choisi : 
  EnvoiActeCCAMCode4_Remplacement si à oui on remplace automatiquent l'acte de code 4
  par un acte de la liste de l'option EnvoiActeCCAMCode4_Remp_CCAM
  */
  
  /*
  if ( $options->getOption("EnvoiActeCCAMCode4_Remplacement") ) {
    $liste_CCAM = explode (",",$options->getOption("EnvoiActeCCAMCode4_Remp_CCAM"));
    $message.= "<br>Remplacement de l'acte '$codeActe' par "; 
    for ( $i = 0 ; isset($liste_CCAM[$i]) ; $i++ ) {
      // On affecte l'acte CCAM aux patient
      $this->add1Acte($liste_CCAM[$i]);
      $message.="'$liste_CCAM[$i]' ";
    }
  }
  */
  ...
  return $message;
/*
---------------------------------------EnvoiActeCCAMCode4_Remplacement
Remplacement de l'acte CCAM de code 4 si anesthésiste non précisé par une liste prédefinie d'actes
---------------------------------------EnvoiActeCCAMCode4_Remp_CCAM
liste prédefinie d'actes pour le remplacement de l'acte CCAM de code 4
*/
}
else{
	unset($param);
...
function tableauValeurs($requete,$param="",$lignePresentation=""){
	...
	//eko($res);
	if ( $requete == "CCAM_getListeCatDiag" )
    //eko($res);
...
function writeBALSorti($cw="",$idEvent=""){
  ...
  for ($i=0;isset($res["identifiant"][$i]);$i++){
    ...
    if ( $this->dest_attendue == "Sans avis médical" )
      $clDateDeb=new clDate($dtFinInterv);
    else
  ...
  if ( $options->getOption ( 'Activite4_EnvoiActeCCAMCode4') == 0 )
  ...
  // Gére les consultations non facturable
  
  /* Jusqu'ici les actes non facturable étaient mis de côté
  pour pouvoir appliqué les régles de facturation
  sur les consultations facturables exemple majoration suivant l'âge du patient
  
  maintenant on va rechercher sur l'ensemble des actes envoyer à la facturation
  si presence ou pas d'actes CCAM et consultation facturable.
  
  si rien on affecte une consultation non facturable ......
  si les deux fac et non fac on efface le non facturable .......
  */
...
/******************************************************************************/
  ...
  // $this->dest_attendue
  // On règle le problème de l'ATU concernant les sorties sans avis médical.
  // On génére un ATU suivant les règles de facturation et on mettra
  // le nom du medecin définie dans les options.
  // Ceci dans la fonction gestionEnvoiActeATU
  ...
  if ( $res2[INDIC_SVC][2] == 0 ) {
    // Presence d'aucun acte donc sorti sans acte.
    if ( $this->dest_attendue == "Sans avis médical" ) {
      ...
      $param[idDomaine]                         = CCAM_IDDOMAINE;
      $req                                      = new clResultQuery ;
      $resATU                                   = $req->Execute("Fichier","CCAM_getNomMedURG",$param,"ResultQuery");
      //eko($resATU);
      $paramModif["envoi_nomIntervenant"]       = $resATU["nomitem"][0];
    }
    else {
      $paramModif["envoi_matriculeIntervenant"] = $options->getOption('SansAvisMedical_Medecin_ATU');
      unset($param);
...
/******************************************************************************/
  ...
  // Pour sortie sans avis médical le patient est dans la table patients_sortis
  if ( strcmp($patient->getDateExamen(),"0000-00-00 00:00:00") == 0 ) {
    $ele[dt_examen] = $patient->getDateSortie( );
    $patient->setPatient2 ( $ele );
    }
  ...
  if ( $options->getOption('EnvoiATURegleFacturation') ) {
    ...
    if ( $fusion == 1 ) {
      ...
      if ( $res["uf"][0] == $options->getOption ( 'numUFUHCD' ) OR $res["uf"][0] == $options->getOption ( 'numUFUHCDrepere' ) )
        ...
        $info_ifInUHCD = 0;
      if  ( $info_TypeDestination == "H" || $info_TypeDestination == "X" || $info_ifInUHCD == 1  || ! $res['valide'][0] )
        $test = 1;
      else
        $test = 0;
    }
      ...
      if ( $patient->getTypeDestination( ) == "H" ||  $patient->getTypeDestination( ) == "X" || $patient->ifInUHCD ( ) == 1 || ! $patient->getValide ( ) )
        $test = 1;
      else
        $test = 0;
    ...
    if ( $test ) {
      ...
      $paramModif["envoi_nomIntervenant"]       = $resATU["nomitem"][0];
        ...
        if ( $this->dest_attendue == "Sans avis médical" )
          $paramRq["dateDemande"]       =   $patient->getDateSortie( );
        else
          $paramRq["dateDemande"]       =   $this->dateEvent;
  ...
  for ( $i = 0 ; $i < $resultat[INDIC_SVC][2] ; $i++ ) {
      ...
      if ( substr($resultat["codeActe"][$i],0,4) == "NGAP" ) {
        ...
        if ( $resultat['cotationNGAP'][$i] == 'ATU 1' ) {
        	$paramModif["envoi_facturation"] = ($options->getOption('EnvoiATU')?(( $envoi["envoi_facturation"][0] == 0  || (is_object($patient) AND ! $patient->getValide ( )))?'0':$envoi[envoi_facturation][0]):0);
        // DBFIN3
        } else $paramModif["envoi_facturation"] = ($options->getOption('EnvoiNGAP')?$envoi[envoi_facturation][0]:0);
		    $requete                         = new clRequete(CCAM_BDD,"ccam_cotation_actes",$paramModif);
...
eko("DEQP003");
  ...
  if ( $tarifTotalActesCCAM >= $tarifTotalActesNGAP AND $tarifTotalActesCCAM ) {
    eko ("CCAM");
    //eko ($res[INDIC_SVC][2]);
    for ( $i = 0 ; $i < $res['INDIC_SVC'][2] ; $i++ ) {
      if ( strcmp($res["type"][$i],"ACTE") == 0 ) {
        ...
        if ( strcmp($cotation[0],"C") == 0 || 
          ...
          if ( $resultat['INDIC_SVC'][2] == 1 ) {
            ...
            $paramVerif["cw"] = "idEvent=".$idEvent." and type='ACTE' and codeActe='".$resultat[CODE][0]."'";
            $verif            = $requete->Execute("Fichier","CCAM_getActesDiagsCotation",$paramVerif,"ResultQuery");
            //eko($verif);
            if ( $verif["codeActivite4"][0] == "O" and $verif["envoi_facturation"][0] == 0 )
              $paramModif["envoi_facturation"] = 0;
            else
              $paramModif["envoi_facturation"] = 1;
...
for ($i=0;isset($res[identifiant][$i]);$i++){ 
  ...
  elseif (substr($res[codeActe][$i],0,4)=="NGAP") {
    ...
    if ( strcmp($Majoration_A_Effectuee,"Non")==0 ) {   
        ...
        //cervetti , 6 janvier 2009, commentaire ligne suivante qui parait fausse
        // pour les samedis
        //if ( date("w",$dateActe)==6 && $heure>=8 && strcmp($Majoration_A_Effectuee,"Non")==0 ) {
        //  $MajorationNGAP1 = "F";$Majoration_A_Effectuee = "Oui";}
